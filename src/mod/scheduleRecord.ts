...
import { termCodes, sharedClient } from '../soc/umichApi'
...
const ignoredClasses: Array<[Course, number]> = [
  [new Course('UARTS', 150), 1],
  [new Course('ENGR', 100), 210],
  [new Course('UARTS', 175), 1]
]

let activeScheduleDisplays: Array<{
  userId: string
  term: number
...
const mod = host.module('scheduleRecord')
  ...
  if (intx.isButton()) {
    ...
    if (tokens[0] === 'schedule-add-section-submit') {
      ...
      if (enrollment !== undefined) {
        await intx.reply({
          ephemeral: true,
          content: `:white_check_mark: Successfully added ${course.toString()}, section ${zeroPad(section)} (${
            sectionInfo.type.toString()
        ...
        await intx.reply({
          ephemeral: true,
          content: `:eyes: ${course.toString()}, section ${zeroPad(section)} is already in your ${term} schedule.`
        })
      }
    }
    ...
    if (tokens[0] === 'schedule-remove-section-submit') {
      ...
      await intx.update({
        content: `:white_check_mark: Successfully removed ${course.toString()}, section ${zeroPad(
          sectionNumber
...
mod.slash('schedule', 'See and edit your class schedule', {
  build: (bx: SlashCommandBuilder) => bx.addStringOption((opt) =>
    opt
      .setName('term')
...
mod.slash('schedule-set-classes', 'Set the entire list of class numbers representing your schedule', {
  ...
  async check (ix): Promise<void> {
    ...
    if (numbers.length > 20) {
      throw new Error(`:x: Too many classes (20 max, ${numbers.length} supplied)`)
    }
    for (const num of numbers) {
      if (isNaN(parseInt(num, 10))) {
        throw new Error(stripMarkdownTag`:x: \`${num}\` is not a valid integer.`)
      }
    }
  },
  ...
  async run (ix: ChatInputCommandInteraction) {
    ...
    const progressData: Array<[Section<true>, Course] | Error | null> = new Array(classNumbers.length).fill(null)
    const replyUpdates = async (): Promise<void> => {
      try {
        ...
        if (done) {
          const sectionsCourses = progressData.filter(Array.isArray) as Array<[Section<true>, Course]>
          await ix.editReply({
          ...
          setTimeout(() => {
            // TODO deal with failures more systematically
            replyUpdates().catch(console.error)
          }, 250)
        }
      } catch (e: unknown) {
        console.error(e)
...
mod.slash('schedule-peers', 'Find others who are taking or have taken the classes you\'re taking.', {
  ...
  async check (ix): Promise<void> {
    ...
    if (!Object.keys(termCodes).includes(term)) {
      throw new Error(`:x: ${term} is not a valid term.`)
    }
    if (!(await store.hasEnrollments(BigInt(ix.user.id), termCodes[term as Term]))) {
      throw new Error(`:x: Your ${term} schedule is empty. Use \`/schedule\` or \`/schedule-set-classes\` to add classes to it.`)
    }
  },
...
mod.slash('schedule-clear', 'Clears your entire schedule for a given term.', {
  build: (builder: SlashCommandBuilder) => builder.addStringOption((opt) => opt
    .setName('term')
    .setDescription(
...
mod.slash('schedule-notify', 'Indicate whether you want to be notified about new academic peers.', {
  build: (builder: SlashCommandBuilder) =>
...
mod.slash('schedule-share', 'Sends a DM of your schedule to another user', {
  build: (builder: SlashCommandBuilder) => builder
    .addUserOption((opt) =>
      opt
        .setName('recipient')
...
async function sendNotifications (client: Client, notifications: Array<{ id: bigint, message: string }>): Promise<void> {
  await Promise.all(
...
async function fetchPeerInfo (studentId: bigint, termCode: number): Promise<store.PeerInfo> {
...
async function updateDisplayedSchedules (userId: string, term: Term): Promise<void> {
  await Promise.all(
    activeScheduleDisplays
      ...
      .map(async (disp) => {
        ...
        await disp.interaction.editReply({
...
function scheduleActionRow (classes: unknown[], term: Term): ActionRowBuilder<ButtonBuilder> {
...
function meetingToLine (mtg: Meeting<true>): string {
...
async function peerNotifications (
  ...
  redundant = false
): Promise<Array<{ id: bigint, message: string }>> {
  if (
    ignoredClasses.some(
  ...
  interface DirectMessageCandidate { id: bigint, message: string, priority: number }
...
async function peerEmbeds (peers: store.PeerInfo, term: Term): Promise<EmbedBuilder[]> {
  const isOrAre = (amount: number): string => (amount === 1 ? 'is' : 'are')
...
async function scheduleEmbed (
  userId: string,
  classes: Array<[Section<true>, Course]>,
  term: Term
