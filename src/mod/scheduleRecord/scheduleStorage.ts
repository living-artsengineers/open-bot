...
export async function getEnrollments(user: bigint, term: number): Promise<[Section<true>, Course][]> {
...
export async function clearEnrollment(user: bigint, term: number) {
...
export async function addEnrollment(user: bigint, term: number, course: Course, section: number) {
  const rowData = {
    studentId: user,
    term,
    courseCode: course.toString(),
...
export async function removeEnrollment(user: bigint, term: number, course: Course, section: number) {
  await client.enrollment.deleteMany({
    where: {
      studentId: user,
      term,
...
export function rollUpAsObjectOfArrays<K extends string | number | symbol, V>(items: [K, V][]): Record<K, V[]> {
