...
export interface ISocApiClient {
  ...
  fetchAllSections: (course: Course, termCode: number) => Promise<Array<Section<false>>>
...
export class UMichSocApiClient implements ISocApiClient {
  ...
  private readonly axios: AxiosInstance = axios.create({ baseURL: endpointPrefix })
  ...
  async fetchAllSections (course: Course, termCode: number): Promise<Section[]> {
    ...
    return (arrayify(sections) as SectionJson[]).map(parseSection)
  }
  ...
  async getSectionBySectionNumber (
    ...
    if (this.sectionCache[termCode][course.toString()] === undefined) { this.sectionCache[termCode][course.toString()] = {} }
  ...
  return {
    ...
    meetings: arrayify(sectionJson.Meeting)
      .map((mtx) => ({
        ...
        startTime: null,
        endTime: null,
        location: mtx.Location !== undefined && mtx.Location !== 'TBA' ? mtx.Location : null,
        ...parseMeetingTimes(mtx.Times)
      }))
      .filter((it) => it.days.size > 0) as Array<Meeting<Loc>>,
